...
//	SELECT id FROM ListaObservacao obj LEFT JOIN lista_obs_jogador ON id_lista_observacao=id where id=1 and id_jogador in (1)
	@Query("SELECT obj.id FROM ListaObservacao obj LEFT JOIN obj.jogadores jog WHERE obj.id= :id AND jog=:jogador")
	List<ListaObservacao> findByIdAndJogadoresIn(Integer id, Jogador jogador);
